...
public class JavaUtils {
     ...
     * @throws ClassNotFoundException
     */
    public static Class loadJavaType( String typeName,
                                      ClassLoader classLoader ) throws ClassNotFoundException {
        if (classLoader == null) classLoader = Thread.currentThread().getContextClassLoader();

        Class javaType = primitiveNames.get(typeName);
        if (javaType == null) javaType = getArray(typeName, classLoader);

        if (javaType == null) javaType = classLoader.loadClass(typeName);
    ...
    private static Class getArray( String javaType,
                                   ClassLoader loader ) throws ClassNotFoundException {
        if (javaType.charAt(0) == '[') return getArrayFromJVMName(javaType, loader);

        if (javaType.endsWith("[]")) return getArrayFromSourceName(javaType, loader);

        return null;
    }

    private static Class getArrayFromJVMName( String javaType,
                                              ClassLoader loader ) throws ClassNotFoundException {
        Class componentType;
    ...
    private static Class getArrayFromSourceName( String javaType,
                                                 ClassLoader loader ) throws ClassNotFoundException {
    ...
    public static Class getPrimitiveType( Class javaType ) {
        if (javaType == Integer.class) return int.class;
        ...
        if (javaType == Character.class) return char.class;
    ...
    public static Object syncArray( Object array,
                                    Class target ) {
     ...
     * @return boolean
     */
    public static boolean isAssignableFrom( Class dest,
                                            Class src ) {
        if (dest == null) throw new IllegalArgumentException("Destination class cannot be null");
        if (src == null) throw new IllegalArgumentException("Source class cannot be null");
    ...
    public static String convertJVMNameToSourceName( String typeName,
                                                     ClassLoader loader ) {
    ...
    /**
     * Converts a JVM external name to a JVM signature name. An external name is that which is returned from
     * {@link Class#getName()} A signature name is the name in class file format.
     * <p>
     * For example:
     * <p>
     * [java.lang.Object
    ...
    public static String capitalize( String source ) {
        ...
        if (Character.isUpperCase(source.charAt(0))) return source;
    ...
    /**
     * This method tests for retro translation by searching for a known problem where Class does not implement Type. If this is
     * true, then code must never cast a Class to a Type.
     * 
    ...
    public static boolean isJBossRepositoryClassLoader( ClassLoader loader ) {
        ...
        while (!clazz.getName().startsWith("java")) {
            if ("org.jboss.mx.loading.RepositoryClassLoader".equals(clazz.getName())) return true;
    ...
    /**
     * Clears black lists on a JBoss RepositoryClassLoader. This is somewhat of a hack, and could be replaced with an integration
     * module. This is needed when the following order of events occur.
     * <ol>
